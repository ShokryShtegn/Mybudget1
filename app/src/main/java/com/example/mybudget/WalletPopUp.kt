...
class WalletPopUp : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(com.example.mybudget.R.layout.wallet_dialog)

        // var l: LinearLayout = findViewById(R.)
        // Get the data
        val bundle = intent.extras

        val context = this@WalletPopUp
                ...
                if (dU[j].id == ID_Of_User.toInt()) {
                    ...
                    if (data.size == 0) {
                        if ((wallet_name.text.toString().isNotEmpty()) &&
                            ...
                            db.insertWallet(wallet, ID_Of_User.toInt())
                        ...
                        for (i in 0..(data.size - 1)) {
                            if ((wallet_name.text.toString().isNotEmpty()) &&
                                (card_type.toString().isNotEmpty()) &&
                                (currency_type.toString().isNotEmpty()) &&
                                (balance.text.toString().isNotEmpty() && (!wallet_name.text.toString().equals(data[i].wallet_name)))
                            ) {
